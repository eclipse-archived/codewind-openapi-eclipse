...
public class CommandRunner {
	...
	public CommandRunner(List<String> args, String commandName) {
		...
		args_.addAll(args);
		lineSeparator_ = System.getProperty("line.separator"); //$NON-NLS-1$
	...
	public IStatus run() {
		...
		try {
			...
			if (javaHomePath != null) {
				...
				environment.put("JAVA_HOME", javaHomePath.getPath()); //$NON-NLS-1$
				...
				if(javaExecutablePath != null && javaExecutablePath.exists()) {
					...
					if (keySet.contains("PATH")) { //$NON-NLS-1$
						environment.put("PATH", javaBinDirectory.getPath() + File.pathSeparator + environment.get("PATH")); //$NON-NLS-1$ //$NON-NLS-2$
					} else if (keySet.contains("Path")) { //$NON-NLS-1$
						environment.put("Path", javaBinDirectory.getPath() + File.pathSeparator + environment.get("Path") ); //$NON-NLS-1$ //$NON-NLS-2$
					} else {
						environment.put("PATH", javaBinDirectory.getPath()); //$NON-NLS-1$
					}
			...
			Activator.log(IStatus.INFO,"Codegen result:"); //$NON-NLS-1$
			Activator.log(IStatus.INFO,"RC = " + rc + "\n" + outputBuffer.toString()); //$NON-NLS-1$ //$NON-NLS-2$
			if (rc != 0) {
				...
				String consoleOutputNew = outputBufferText.replace("\r", ""); //$NON-NLS-1$ //$NON-NLS-2$
	...
	private static File getJavaBinaryPath(final File homeDir) {
		...
		if (homeDir != null && homeDir.exists()) {
			...
			boolean isWindows = osName.indexOf("win") >= 0; //$NON-NLS-1$
